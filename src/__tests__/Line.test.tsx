...
test("a line segment should have a different start and end", async () => {
    ...
    expect(() => new LineSegment(p1, p2)).toThrow();
});
...
test("is point near segment", async () => {
    const l = new LineSegment(new Point(0, 0), new Point(6, 8));
    const p1 = new Point(0, 0);
    const p2 = new Point(6, 8);
...
test("find intersection point of two lines", async () => {
    ...
    let l1 = new LineSegment(p1, p2);
    let l2 = new LineSegment(p3, p4);
    expect(LineSegment.findIntersectionPointOfTwoLines(l1, l2, false)).toBeNull();
    expect(LineSegment.findIntersectionPointOfTwoLines(l1, l2, true)).toBeNull();
    ...
    expect(LineSegment.findIntersectionPointOfTwoLines(l1, l2, false)?.equals(i)).toBeTruthy();
    expect(LineSegment.findIntersectionPointOfTwoLines(l1, l2, true)?.equals(i)).toBeTruthy();
